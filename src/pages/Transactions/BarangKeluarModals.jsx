...
function BarangKeluarFormModal({
    isOpen,
    modalMode,
    formData,
    errors,
            ...
            <form onSubmit={onSubmit}>
                <div className="pr-12">
                    ...
                    <h3 className="text-xl font-semibold text-gray-800 dark:text-white/90">
                        {modalMode === "add" ? "Tambah Barang Keluar" : "Edit Barang Keluar"}
                    </h3>
                ...
                <div className="mt-6 grid grid-cols-1 gap-5 md:grid-cols-2">
                    ...
                    <div>
                        <Label htmlFor="id_master_anggota">Anggota</Label>
                        <select
                            id="id_master_anggota"
                            name="id_master_anggota"
                            value={formData.id_master_anggota}
                            onChange={onFormChange}
                            className={`h-11 w-full rounded-lg border bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs focus:outline-hidden focus:ring-3 dark:bg-gray-900 dark:text-white/90 ${errors.id_master_anggota
                        ...
                        >
                            ...
                            {members.map((member) => (
                                <option key={member.id} value={member.id}>
                                    {member.nomor_anggota} - {member.nama_anggota}
                                </option>
                            ))}
                        </select>
                        {renderSelectError("id_master_anggota")}
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <Input
                            ...
                            error={Boolean(errors.harga_jual)}
                            hint={errors.harga_jual}
                        />
                    </div>
                    <div>
                ...
                <div className="mt-6 grid grid-cols-1 gap-4 rounded-xl border border-gray-100 bg-gray-50 p-4 dark:border-gray-800 dark:bg-white/[0.03] sm:grid-cols-2 lg:grid-cols-4">
                    <DetailItem label="Total" value={formatRupiah(calculation.total_harga_jual)} />
                    <DetailItem label="Sisa Bayar" value={formatRupiah(calculation.sisa_bayar)} />
                    <DetailItem label="Margin" value={formatRupiah(calculation.margin)} />
                    <div>
...
function BarangKeluarDeleteModal({ isOpen, selectedTransaction, onClose, onDelete }) {
    return (
        <Modal
            isOpen={isOpen}
            onClose={onClose}
        ...
        >
            <div className="pr-12">
                ...
                <p className="mt-2 text-sm text-gray-500 dark:text-gray-400">
                    Transaksi <span className="font-medium text-gray-800 dark:text-white/90">{selectedTransaction?.nama_barang}</span> untuk <span className="font-medium text-gray-800 dark:text-white/90">{selectedTransaction?.nama_anggota}</span> akan dihapus dari daftar lokal.
                </p>
            </div>
...
export { BarangKeluarDeleteModal, BarangKeluarFormModal };
